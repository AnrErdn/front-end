      ...
      useEffect(() => {
        const fetchData = async () => {
          try {
            ...
            const historicalsData = await fetchAndCheck("https://mongol-api-rest.vercel.app/historical-tools");
            ...
            const figuresData = await fetchAndCheck("https://mongol-api-rest.vercel.app/figures");
            setFigures(figuresData.figures || []);
        
            const touristsData = await fetchAndCheck("https://mongol-api-rest.vercel.app/tourists");
                ...
                {filteredTourists.map((item) => (
                  ...
                  >
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        <span className="px-3 py-1 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-full text-xs font-medium">
                          Address: {item.address || "No address data"}
                        </span>
                      </div>
                    </div>
                  </div>
      ...
      return (
        <div className="min-h-screen bg-white dark:bg-black text-black dark:text-white">
          ...
          <main className="container mx-auto px-4 py-8">
            ...
            {/* Empty State */}
            {(
              (activeTab === 'clothes' && filteredClothes.length === 0) ||
              (activeTab === 'instruments' && filteredInstruments.length === 0) ||
              (activeTab === 'historicals' && filteredHistoricals.length === 0) ||
              (activeTab === 'ethnics' && filteredEthnics.length === 0) ||
              (activeTab === 'provinces' && filteredProvinces.length === 0) ||
              (activeTab === 'figures' && filteredFigures.length === 0) ||
              (activeTab === 'tourist' && filteredTourists.length === 0)
            ) && (
          ...
          <footer className="bg-white dark:bg-black border-t border-gray-200 dark:border-gray-800 py-8 px-4">
            <div className="container mx-auto">
              <p className="text-center text-gray-500 dark:text-gray-400 text-sm">
                © {new Date().getFullYear()} FCK ALL. All rights reserved.
              </p>
            </div>
          </footer>
        </div>
